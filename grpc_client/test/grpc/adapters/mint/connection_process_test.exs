defmodule GRPC.Client.Adapters.Mint.ConnectionProcessTest do
  ...
  describe "chunk_body/2 (via handle_continue - body chunking behaviour)" do
    setup :valid_connection
    setup :valid_stream_request
    setup :valid_stream_response

    test "streams only window_size bytes and re-enqueues the remainder at the head of the queue",
         %{request_ref: request_ref, state: state} do
      # Window size of 3 bytes — only 3 of the 7 body bytes should be streamed
      state = %{state | conn: %{state.conn | window_size: 3}}

      {_, state, _} =
        ConnectionProcess.handle_call(
          {:stream_body, request_ref, <<1, 2, 3, 4, 5, 6, 7>>},
          self(),
          state
        )
      ...
      # Mint decrements the connection window by the number of bytes streamed
      assert new_state.conn.window_size == 0

      # The un-streamed tail <<4, 5, 6, 7>> must be at the head of the queue
      assert {{:value, {^request_ref, <<4, 5, 6, 7>>, _from}}, _rest} =
               :queue.out(new_state.request_stream_queue)
    end

    test "streams only window_size bytes when body is exactly twice the window and preserves both segments in order",
         %{request_ref: request_ref, state: state} do
      # Window of 2 — first chunk is <<1, 2>>, remainder <<3, 4>> goes back to queue head,
      # then a second payload <<5, 6>> is behind it
      state = %{state | conn: %{state.conn | window_size: 2}}

      {_, state, _} =
        ConnectionProcess.handle_call({:stream_body, request_ref, <<1, 2, 3, 4>>}, self(), state)

      {_, state, _} =
        ConnectionProcess.handle_call({:stream_body, request_ref, <<5, 6>>}, self(), state)
      ...
      # Head of queue must be the un-streamed tail of the first payload
      assert {{:value, {^request_ref, <<3, 4>>, _}}, remaining_queue} =
               :queue.out(new_state.request_stream_queue)

      # Second payload must still be next
      assert {{:value, {^request_ref, <<5, 6>>, _}}, _} = :queue.out(remaining_queue)
    end

    test "streams the full body when it fits exactly within the window size",
         %{request_ref: request_ref, state: state} do
      body = <<10, 20, 30>>
      # Set window size to exactly the body length
      state = %{state | conn: %{state.conn | window_size: byte_size(body)}}

      {_, state, _} =
        ConnectionProcess.handle_call({:stream_body, request_ref, body}, {self(), :tag}, state)

      assert {:noreply, _new_state} =
               ConnectionProcess.handle_continue(:process_request_stream_queue, state)

      # Caller must receive :ok — meaning the full body was streamed in one shot
      assert_receive {:tag, :ok}, 500
    end

    test "streams the full body when body is a single byte equal to window size",
         %{request_ref: request_ref, state: state} do
      state = %{state | conn: %{state.conn | window_size: 1}}

      {_, state, _} =
        ConnectionProcess.handle_call({:stream_body, request_ref, <<42>>}, {self(), :tag}, state)
