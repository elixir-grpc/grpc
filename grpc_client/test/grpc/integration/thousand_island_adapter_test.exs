defmodule GRPC.Integration.ThousandIslandAdapterTest do
  ...
  # Test server for all streaming types
  ...
  describe "ThousandIsland adapter - unary RPC" do
    test "handles simple unary request/response" do
    ...
    test "handles multiple sequential unary calls" do
      run_server([HelloServer], fn port ->
        {:ok, channel} = GRPC.Stub.connect("localhost:#{port}")
  ...
  describe "ThousandIsland adapter - server streaming RPC" do
    test "receives multiple responses from server" do
      run_server([RouteServer], fn port ->
        {:ok, channel} = GRPC.Stub.connect("localhost:#{port}")
  ...
  describe "ThousandIsland adapter - client streaming RPC" do
    test "sends multiple requests and receives single response" do
      run_server([RouteServer], fn port ->
        {:ok, channel} = GRPC.Stub.connect("localhost:#{port}")
  ...
  describe "ThousandIsland adapter - bidirectional streaming RPC" do
    test "exchanges messages bidirectionally" do
      run_server([RouteServer], fn port ->
        {:ok, channel} = GRPC.Stub.connect("localhost:#{port}")
  ...
  describe "ThousandIsland adapter - HTTP/2 protocol validation" do
    test "handles multiple concurrent unary calls on same connection" do
      run_server([HelloServer], fn port ->
        {:ok, channel} = GRPC.Stub.connect("localhost:#{port}")
