    ...
    log =
      capture_log(fn ->
        run_endpoint(CollisionEndpoint, fn port ->
          ...
          {:ok, reply} = CollisionTest.CollisionService.Stub.call(:process_data, channel, req, [])
          assert reply.result == "process_data: test1"
        end)
      end)

    assert log =~ "GRPC.Integration.CollisionLoggingTest.CollisionServer.call(:process_data, ...)"
  end
    ...
    log =
      capture_log(fn ->
        run_endpoint(CollisionEndpoint, fn port ->
          ...
          {:ok, reply} = CollisionTest.CollisionService.Stub.call(:ProcessData, channel, req, [])
          assert reply.result == "ProcessData: test2"
        end)
      end)

    assert log =~ "GRPC.Integration.CollisionLoggingTest.CollisionServer.call(:ProcessData, ...)"
  end
