defmodule GRPC.HTTP2.Frame.Settings do
  @moduledoc false
  ...
  def deserialize(flags, 0, payload) when clear?(flags, @ack_bit) do
    ...
    |> Enum.reduce_while({:ok, %{}}, fn
      ...
      {:ok, {0x02, _value}}, {:ok, _acc} ->
        {:halt,
         {:error, GRPC.HTTP2.Errors.protocol_error(), "Invalid enable_push value (RFC9113§6.5)"}}
      ...
      {:ok, {0x04, value}}, {:ok, _acc} when value > @max_window_size ->
        {:halt,
         {:error, GRPC.HTTP2.Errors.flow_control_error(), "Invalid window_size (RFC9113§6.5)"}}
      ...
      {:ok, {0x05, value}}, {:ok, _acc} when value < @min_frame_size ->
        {:halt,
         {:error, GRPC.HTTP2.Errors.frame_size_error(), "Invalid max_frame_size (RFC9113§6.5)"}}
      ...
      {:error, _rest}, _acc ->
        {:halt,
         {:error, GRPC.HTTP2.Errors.frame_size_error(), "Invalid SETTINGS size (RFC9113§6.5)"}}
    end)
    |> case do
  ...
  def deserialize(flags, 0, _payload) when set?(flags, @ack_bit) do
    {:error, GRPC.HTTP2.Errors.frame_size_error(),
     "SETTINGS ack frame with non-empty payload (RFC9113§6.5)"}
  end

  def deserialize(_flags, _stream_id, _payload) do
    {:error, GRPC.HTTP2.Errors.protocol_error(), "Invalid SETTINGS frame (RFC9113§6.5)"}
  end

  defimpl GRPC.HTTP2.Frame.Serializable do
    @ack_bit 0

    def serialize(%GRPC.HTTP2.Frame.Settings{ack: true}, _max_frame_size),
      do: [{0x4, set([@ack_bit]), 0, <<>>}]

    def serialize(%GRPC.HTTP2.Frame.Settings{ack: false} = frame, _max_frame_size) do
      payload =
        frame.settings
