defmodule GRPC.HTTP2.Frame.Ping do
  @moduledoc false
  ...
  def deserialize(_flags, stream_id, _payload) when stream_id != 0 do
    {:error, GRPC.HTTP2.Errors.protocol_error(), "Invalid stream ID in PING frame (RFC9113§6.7)"}
  end
  ...
  defimpl GRPC.HTTP2.Frame.Serializable do
    ...
    def serialize(%GRPC.HTTP2.Frame.Ping{ack: true} = frame, _max_frame_size),
      do: [{0x6, set([@ack_bit]), 0, frame.payload}]

    def serialize(%GRPC.HTTP2.Frame.Ping{ack: false} = frame, _max_frame_size),
