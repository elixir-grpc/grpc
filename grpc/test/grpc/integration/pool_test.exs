defmodule GRPC.Integration.PoolTest do
  ...
  test "pool replaces a crashed channel immediately to maintain pool_size" do
    run_server(HelloServer, fn port ->
      ...
      old_references = Map.keys(channels)
      assert Enum.count(old_references) == 2

      [{killed_id, wrapped_ch} | _] = Map.to_list(channels)
      conn_pid = wrapped_ch.channel.adapter_payload.conn_pid

      # Wait for the connection to actually be down so the pool's own :DOWN is
      # guaranteed to have been generated, then poll until it has been handled.
      ref = Process.monitor(conn_pid)
      Process.exit(conn_pid, :kill)
      assert_receive {:DOWN, ^ref, :process, ^conn_pid, _}

      new_references = await_channel_replaced(pool_pid, killed_id)

      assert Enum.count(new_references) == 2
      assert (new_references ++ old_references) |> Enum.uniq() |> Enum.count() == 3

      # The pool keeps serving on the replacement channel.
      assert {:ok, %Helloworld.HelloReply{}} =
               Helloworld.Greeter.Stub.say_hello(channel, %Helloworld.HelloRequest{name: "after"})
    end)
  end

  defp await_channel_replaced(pool_pid, killed_id, tries \\ 200)

  defp await_channel_replaced(_pool_pid, _killed_id, 0),
    do: flunk("pool did not replace the crashed channel")

  defp await_channel_replaced(pool_pid, killed_id, tries) do
    %{channels: channels} = :sys.get_state(pool_pid)

    if map_size(channels) == 2 and not Map.has_key?(channels, killed_id) do
      Map.keys(channels)
    else
      await_channel_replaced(pool_pid, killed_id, tries - 1)
    end
  end
