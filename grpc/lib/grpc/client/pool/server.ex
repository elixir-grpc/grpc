defmodule GRPC.Client.Pool.Server do
  ...
  defmodule State do
    ...
    defmodule Channel do
      ...
      defstruct [:id, :open_streams, :channel, :monitor_ref]
  ...
  @impl GenServer
  @spec init(State.t()) :: {:ok, State.t(), {:continue, :open_connections}}
  def init(state) do
    Process.flag(:trap_exit, true)
    # Open connections in a continuation rather than here: each connection is
    # started via DynamicSupervisor.start_child on GRPC.Client.Supervisor, the
    # same supervisor that is synchronously starting this pool. Opening them in
    # init would deadlock that supervisor; the continue runs once it is free.
    {:ok, state, {:continue, :open_connections}}
  end

  @impl GenServer
  def handle_continue(:open_connections, state) do
    {:noreply, Implementation.init(state)}
  end
  ...
  @impl GenServer
  # A :DOWN here is either a connection process we monitor going down (remove the
  # channel and replenish the pool) or a lease caller going down (return its channel).
  def handle_info({:DOWN, ref, :process, _pid, _reason}, state) do
    {state, monitor_refs} = Implementation.handle_down(state, ref)
